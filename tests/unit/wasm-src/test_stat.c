// Test: stat syscall returns correct data for root directory
...
__attribute__((import_module("yos"), import_name("stat")))
int yos_stat(const char* path, void* buf);

// Types matching wasm-compat.h exactly
...
void _start(void) {
    ...
    if (r != 0) {
        write_str("E1: stat failed r=");
        write_num(-r);
        write_str("\n");
        yos_exit(1);
    }

    // Check st_mode is a directory (040xxx)
